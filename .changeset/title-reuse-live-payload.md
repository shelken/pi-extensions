...
本次一并移除为对齐 messages 而堆叠的补丁（`convertToLlm`、`applyContextPruneIndex`、`CacheMissDumper`、`msgFingerprint`）——live payload 已继承这些处理，无需 pi-title 重复。要求 provider 遵守 `onPayload` 契约（调 `options.onPayload` + 用返回值替换发送体 + 替换后不再强写），pi 内置 12 个 provider 均遵守；不遵守的自定义 provider 退化为 buildParams 产物原样。
